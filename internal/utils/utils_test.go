...
func TestGetPostgresConnectionString(t *testing.T) {
	...
	if expectedConnectionString != actualConnectionString {
		t.Fatalf("actual: '%s' != expected: '%s'", actualConnectionString, expectedConnectionString)
	}
}
...
func TestGetRequiredEnvVariableFound(t *testing.T) {
	...
	if value != fakeEnvVarValue {
		t.Fatalf("actual: '%s' != expected '%s'", value, fakeEnvVarValue)
	}
...
func TestExitWithError(t *testing.T) {
	...
	if !typeAssertionOk && resultingError.Success() {
		t.Fatalf("expected exit with status '%d' and error '%s'", exitStatus, testError)
	}
}
